...
segmented.to_csv('C:/mtu/project/4HourlyFeatures.csv', index=False)
...
grouped = final.groupby(['Category', 'segment'])['activity'].mean().reset_index()
pivoted = grouped.pivot(index='segment', columns='Category', values='activity')
plt.plot(pivoted.index, pivoted['Control'], label='Control')
plt.plot(pivoted.index, pivoted['Depressive'], label='Depressive')
plt.plot(pivoted.index, pivoted['Schizophrenic'], label='Schizophrenic')
...
plt.ylabel('Average Activity')
plt.title('Average Activity in 4 hourly segments over 24 hours')
plt.legend()
plt.gcf().set_size_inches(12, 6)

plt.savefig('AverageActivityPer4hrSegement.png', dpi=300)
plt.show()
